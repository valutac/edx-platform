...
class ProgramProgressMeter(object):
    ...
    def programs_by_course_run(self):
        ...
        """
        programs_by_course_run = defaultdict(list)
            ...
            for course in program['courses']:
                for course_run in course['course_runs']:
                    ...
                    if course_run_id in self.course_run_ids:
                        program_list = programs_by_course_run[course_run_id]
                        if program not in program_list:
                            program_list.append(program)

        # Sort programs by title for consistent presentation.
        for program_list in programs_by_course_run.itervalues():
            program_list.sort(key=lambda p: p['title'])

        return programs_by_course_run
    ...
    def engaged_programs(self):
        ...
        """
        programs_by_course_run = self.programs_by_course_run()

        programs = []
        for course_run_id in self.course_run_ids:
            for program in programs_by_course_run[course_run_id]:
                # A set would be ideal here, but dicts aren't a hashable type.
    ...
    def completed_course_runs(self):
        ...
        """
        course_run_certificates = certificate_api.get_certificates_for_user(student.username)
        return [
